#!/usr/bin/env python3
"""
Google Drive Uploader for downloaded books (Codespaces version).
Uploads books from the downloads folder to Google Drive and updates the database with shareable links.
This version is designed to work with GitHub Codespaces.
"""
...
def authenticate_google_drive():
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            # For Codespaces, we'll use a manual approach
            ...
            print("Authentication required. Please visit the following URL to authorize the application:")
            print(auth_url)
            print("\nAfter authorizing, you'll receive a code. Please save it to a file named 'auth_code.txt' in this directory.")
            print("Then run this script again with the --use-code flag.")
            return None
...
def authenticate_with_code():
    """Authenticate using a pre-saved authorization code."""
    ...
    if not os.path.exists('auth_code.txt'):
        print("Error: No authorization code file found.")
        print("Please save your authorization code to 'auth_code.txt'.")
        return None
...
def upload_downloaded_books_to_drive(use_code=False):
    ...
    # Authenticate
    if use_code:
        service = authenticate_with_code()
    else:
        service = authenticate_google_drive()
...
if __name__ == "__main__":
    import sys
    use_code = '--use-code' in sys.argv
    upload_downloaded_books_to_drive(use_code)